...
export class RCloneEncryptionProvider implements EncryptionProvider {
  readonly id = "rclone-base64";
  private cipher: CipherRclone;

  constructor(password: string) {
    this.cipher = new CipherRclone(password, 5);
  }

  async encryptContent(content: ArrayBuffer): Promise<ArrayBuffer> {
    const res = await this.cipher.encryptContentByCallingWorker(content);
    if (res === undefined) {
  ...
  async decryptContent(content: ArrayBuffer): Promise<ArrayBuffer> {
    const res = await this.cipher.decryptContentByCallingWorker(content);
    if (res === undefined) {
  ...
  async encryptName(name: string): Promise<string> {
    const res = await this.cipher.encryptNameByCallingWorker(name);
    if (res === undefined) {
  ...
  async decryptName(name: string): Promise<string> {
    const res = await this.cipher.decryptNameByCallingWorker(name);
    if (res === undefined) {
  ...
  getSizeFromOrigToEnc(size: number): number {
    return getSizeFromOrigToEnc(size);
  }

...
export class OpenSSLEncryptionProvider implements EncryptionProvider {
  readonly id = "openssl-base64";
  ...
  async decryptName(name: string): Promise<string> {
    if (name.startsWith(MAGIC_ENCRYPTED_PREFIX_BASE32)) {
      // backward compatible with the openssl-base32
      try {
        const res = await decryptBase32ToString(name, this.password);

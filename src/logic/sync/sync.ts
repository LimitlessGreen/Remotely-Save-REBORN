...
export async function syncer(
  ...
  notice?: (source: SyncTriggerSourceType, message: string) => void,
  ...
  try {
    ...
    const fsEncrypt = new FakeFsEncrypt(
      fsRemote,
      settings.password,
      settings.encryptionMethod ?? "unknown"
    );
    ...
    const mixedEntities: MixedEntity[] = buildMixedEntities(
      localEntities,
      remoteEntities,
      prevSyncRecords,
      settings
...
function buildMixedEntities(
  local: Entity[],
  remote: Entity[],
  _prev: Record<string, Entity>,
  _settings: RemotelySavePluginSettings
): MixedEntity[] {
  const mixed: Record<string, MixedEntity> = {};
  console.info(
    `building mixed entities for ${local.length} local and ${remote.length} remote`
  );
  ...
  const result = Object.values(mixed)
    .map((m) => {
      if (m.local && !m.remote) {
        ...
        const localMtime = m.local.mtimeCli ?? 0;
        const remoteMtime = m.remote.mtimeSvr ?? 0;
        if (localMtime > remoteMtime) {
          m.decision = "local_is_modified_then_push";
          console.debug(
            `[SYNC] ${m.key}: Local is newer (${localMtime}) than Remote (${remoteMtime}) -> PUSH`
          );
        } else if (localMtime < remoteMtime) {
          m.decision = "remote_is_modified_then_pull";
          console.debug(
            `[SYNC] ${m.key}: Remote is newer (${remoteMtime}) than Local (${localMtime}) -> PULL`
          );
        } else {
          m.decision = "equal";
...
async function processMixedEntity(
  mixed: MixedEntity,
  localFs: FakeFsLocal,
  remoteFs: FakeFsEncrypt,
  _db: InternalDBs,
  _settings: RemotelySavePluginSettings
) {
  if (
    mixed.decision === "local_is_created_then_push" ||
    mixed.decision === "local_is_modified_then_push"
  ) {
    ...
    if (mixed.key.endsWith("/")) {
      ...
      const mtime = mixed.local?.mtimeCli ?? Date.now();
      const ctime = mixed.local?.ctimeCli ?? mtime;
      await remoteFs.writeFile(mixed.key, content, mtime, ctime);
    }
  } else if (
    mixed.decision === "remote_is_created_then_pull" ||
    mixed.decision === "remote_is_modified_then_pull"
  ) {
    ...
    if (mixed.key.endsWith("/")) {
      ...
      const mtime = mixed.remote?.mtimeSvr ?? Date.now();
      await localFs.writeFile(mixed.key, content, mtime, mtime);
    }
  }
}

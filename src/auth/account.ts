...
import {
  ...
  type OfficialAccountConfig as AccountConfig,
  type FeatureInfo,
  type OFFICIAL_ACCOUNT_FEATURE_TYPE as FEATURE_TYPE,
} from "../../src/core/baseTypes";
import {
  COMMAND_CALLBACK_PRO,
  OFFICIAL_ACCOUNT_CLIENT_ID,
  OFFICIAL_ACCOUNT_WEBSITE,
} from "../../src/core/baseTypes";
import { codeVerifier2CodeChallenge } from "./oauth2_official";

const site = OFFICIAL_ACCOUNT_WEBSITE;
...
export const generateAuthUrlAndCodeVerifierChallenge = async (
  ...
  const appKey = OFFICIAL_ACCOUNT_CLIENT_ID ?? "cli-"; // hard-code
...
export const getAndSaveAccountFeatures = async (
  config: AccountConfig,
  ...
  const res = {
    proFeatures: features.map(
      (i) =>
        ({
          featureName: i as FEATURE_TYPE,
          enableAtTimeMs: 1e12,
          expireAtTimeMs: 3e12,
        } as FeatureInfo)
    ),
  };
  config.enabledFeatures = res.proFeatures;
  await saveUpdatedConfigFunc?.();
  return res;
  ...
  config.enabledFeatures = rsp2.proFeatures;
  await saveUpdatedConfigFunc?.();
  return rsp2;
};

export const getAndSaveAccountEmail = async (
  config: AccountConfig,
  pluginVersion: string,
  saveUpdatedConfigFunc: () => Promise<any> | undefined
) => {
 ...
 * @returns
 */
export const checkAccountRunnableAndFixInplace = async (
  config: RemotelySavePluginSettings,
  pluginVersion: string,
  saveUpdatedConfigFunc: () => Promise<any> | undefined
): Promise<true> => {
  console.debug(`checkAccountRunnableAndFixInplace`);

  // many checks if status is valid

  // no account
  if (config.officialAccount === undefined || config.officialAccount.refreshToken === undefined) {
    throw Error(`you need to "connect" to your account to use features`);
  }
  ...
  for (const f of config.officialAccount.enabledFeatures) {
    ...
    if (tooFarInTheFuture || alreadyExpired) {
      console.info(
        `the feature is too far in the future and has expired, check again.`
      );
      await getAndSaveAccountFeatures(
  ...
  if (config.conflictAction === "smart_conflict") {
    if (
      config.officialAccount.enabledFeatures.filter(
      ...
      errorMsgs.push(
        `You're trying to use "smart conflict" feature but you haven't subscribe to it.`
      );
    }
  } else {
    // good to go
  ...
  for (const { feature, service, name } of toChecked) {
    ...
    if (config.serviceType === service) {
      if (
        config.officialAccount.enabledFeatures.filter((x) => x.featureName === feature)
          .length === 1
      ) {
        // good to go
      } else {
        errorMsgs.push(
          `You're trying to use "sync with ${name}" feature but you haven't subscribe to it.`
        );
      }
    } else {
      // good to go

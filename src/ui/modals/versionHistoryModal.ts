...
export class VersionHistoryModal extends Modal {
  ...
  async onOpen() {
    ...
    const container = contentEl.createDiv({ cls: "remotely-save-version-history-container" });
    this.sidebarEl = container.createDiv({ cls: "remotely-save-version-sidebar" });
    this.mainEl = container.createDiv({ cls: "remotely-save-version-main" });

    // Header for main area
    const header = this.mainEl.createDiv({ cls: "remotely-save-version-header" });

    const headerTop = header.createDiv({ cls: "remotely-save-version-header-top" });
    this.headerTitleEl = headerTop.createDiv({ text: "Select a version to compare" });
    this.restoreBtnEl = headerTop.createEl("button", {
        text: this.plugin.i18n.t("modal_version_browser_restore" as TransItemType),
        cls: "mod-cta"
    });
    this.restoreBtnEl.style.display = "none";
    this.restoreBtnEl.addEventListener("click", () => this.restoreSelectedVersion());

    this.diffAreaEl = this.mainEl.createDiv({ cls: "remotely-save-diff-area" });
    this.diffAreaEl.createDiv({ cls: "remotely-save-loader", text: "Loading versions..." });

    try {
      this.localContent = await this.app.vault.read(this.file);
      await this.loadVersions();
    } catch (err) {
      this.diffAreaEl.setText("Error initializing: " + (err instanceof Error ? err.message : String(err)));
    }
  }
  ...
  private async loadVersions() {
    try {
      ...
      if (this.versions.length > 0) {
        ...
        this.diffAreaEl.setText(this.plugin.i18n.t("modal_version_browser_no_versions" as TransItemType));
      }
    } catch (err) {
      this.diffAreaEl.setText("Error loading versions: " + (err instanceof Error ? err.message : String(err)));
    }
  }
  ...
  private renderSidebar() {
    ...
    sortedDays.forEach(day => {
      ...
      this.sidebarEl.createDiv({ cls: "remotely-save-version-day-header", text: headerText });

      // Versions for this day
      groups[day].forEach(version => {
        const item = this.sidebarEl.createDiv({ cls: "remotely-save-version-item" });
        ...
        item.createSpan({ cls: "remotely-save-version-item-time", text: timeStr });
        item.createSpan({
  ...
  private async updateDiff() {
    ...
    this.diffAreaEl.createDiv({ cls: "remotely-save-loader", text: "Loading comparison..." });
    ...
    try {
      ...
      const selectedIndex = this.versions.findIndex(v => v.versionId === this.selectedVersion?.versionId);
      const prevVersion = selectedIndex !== -1 && selectedIndex < this.versions.length - 1
      ...
      if (prevVersion) {
        const baseBuffer = await client.readFile(this.file.path, prevVersion.versionId);
      ...
      const remoteBuffer = await client.readFile(this.file.path, this.selectedVersion.versionId);
      const remoteText = new TextDecoder().decode(remoteBuffer);

      this.renderDiffInternal(baseContent, remoteText);

      const targetLabel = (window as any).moment(this.selectedVersion.mtimeSvr).format("LLLL");
      this.headerTitleEl.setText(`Changes in version from ${targetLabel}`);
      this.diffAreaEl.createEl("div", {
        text: `Comparing with previous version from ${baseLabel}`,
        cls: "remotely-save-diff-unchanged",
        attr: { style: "font-size: 0.8em; margin-bottom: 10px; border-bottom: 1px solid var(--background-modifier-border); padding-bottom: 5px;" }
      }, (el) => {
      ...
      this.diffAreaEl.setText("Error loading content: " + (err instanceof Error ? err.message : String(err)));
    }
  }
  ...
  private renderChangedBlock(removedText: string, addedText: string) {
    ...
    if (removedLines.length === addedLines.length) {
      for (let i = 0; i < removedLines.length; i++) {
        ...
        const rLine = this.diffAreaEl.createDiv({ cls: "remotely-save-diff-line remotely-save-diff-removed" });
        rLine.createSpan({ text: "- ", cls: "remotely-save-diff-prefix" });
        wordDiffs.forEach(part => {
          if (part.removed) {
            rLine.createSpan({ text: part.value, cls: "remotely-save-diff-word-removed" });
          } else if (!part.added) {
        ...
        const aLine = this.diffAreaEl.createDiv({ cls: "remotely-save-diff-line remotely-save-diff-added" });
        aLine.createSpan({ text: "+ ", cls: "remotely-save-diff-prefix" });
        wordDiffs.forEach(part => {
          if (part.added) {
            aLine.createSpan({ text: part.value, cls: "remotely-save-diff-word-added" });
  ...
  private renderLineBlock(part: { value: string, added?: boolean, removed?: boolean }) {
  ...
  private async restoreSelectedVersion() {
    ...
    try {
      ...
      const content = await client.readFile(this.file.path, this.selectedVersion.versionId);
      await this.app.vault.modifyBinary(this.file, content);
      new Notice(this.plugin.i18n.t("modal_version_browser_notice_restored" as TransItemType));
      this.close();
    } catch (err) {
      new Notice("Failed to restore: " + (err instanceof Error ? err.message : String(err)));
    }
  }

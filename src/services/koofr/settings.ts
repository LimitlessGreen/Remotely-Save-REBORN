...
import { DEFAULT_KOOFR_CONFIG, generateAuthUrl } from "./fsKoofr";

export class KoofrSettings extends BaseSettingsManager {
  render(containerEl: HTMLElement) {
    const root = containerEl.createDiv({ cls: "koofr-settings-section" });
    root.toggleClass("koofr-hide", this.plugin.settings.serviceType !== "koofr");
  ...
  private addAuthSection(el: HTMLElement) {
    ...
    const refresh = () => {
      ...
      new Setting(area)
        .setName(linked ? this.t("settings_koofr_revoke") : this.t("settings_koofr_auth"))
        .addButton(btn => btn
          .setButtonText(linked ? this.t("settings_koofr_revoke_button") : this.t("settings_koofr_auth_button"))
          .onClick(async () => {
            if (linked) {
              ...
              window.open(generateAuthUrl(this.plugin.settings.koofr.api, true));
            }
          })
        );
    };

...
export const generateAuthUrl = async (_locationId: number = 1) => {
...
class RawPCloudFs implements RawFs {
    ...
    const traverse = async (folderId: number, path: string) => {
      ...
      for (const item of res.contents || []) {
        ...
        list.push({
          ...
          keyRaw: key,
          sizeRaw: item.size || 0,
          mtimeSvr: item.modified
            ? Date.parse(item.modified).valueOf()
            : Date.now(),
        });
        if (isDir) {
  ...
  async stat(fullPath: string): Promise<Entity> {
    ...
    const res = await this.api?.listFolder(
      await this.resolvePathToId(
  ...
  private async ensureInited() {
    ...
    const found = root.contents.find(
      (i: any) => i.name === target && i.isfolder
    );
...
export class PCloudFileSystem extends BaseCloudFs {
  constructor(config: PCloudConfig, vaultName: string) {
    super(
      "pcloud",
      new RawPCloudFs(config, vaultName),

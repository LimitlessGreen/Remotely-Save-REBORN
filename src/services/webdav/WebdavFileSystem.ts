...
import { bufferToArrayBuffer } from "../../utils/misc";
...
if (VALID_REQURL) {
  getPatcher().patch("request", async (options: any): Promise<Response> => {
    ...
    return new Response(
      [101, 103, 204, 205, 304].includes(r.status) ? null : r.arrayBuffer,
      {
        status: r.status,
        statusText,
...
export class RawWebdavFs implements RawFs {
  ...
  private async ensureInited() {
    ...
    try {
      ...
      this.isNextcloud = compliance.compliance.some((c) =>
        c.toLowerCase().includes("nextcloud")
      );
      ...
      this.supportSabrePartial = compliance.compliance.includes(
  ...
  private async getNextcloudFileId(fullPath: string): Promise<string> {
    ...
    if (!fileId) {
      console.error(
        `Nextcloud PROPFIND debug result: ${JSON.stringify(result)}`
      );
  ...
  private getNextcloudVersionsUrl(fileId: string): string {
    ...
    if (davIdx === -1 || parts[davIdx + 1] !== "files")
      throw new Error("Unsupported Nextcloud URL structure");
  ...
  async walk(fullPath: string, partial: boolean): Promise<Entity[]> {
    ...
    if (partial || this.config.depth === "manual_1") {
      ...
      while (q.length > 0) {
        const path = q.pop()!;
        const list = (await this.client.getDirectoryContents(path, {
          deep: false,
        })) as FileStat[];
  ...
  async readFile(fullPath: string, versionId?: string): Promise<ArrayBuffer> {
    ...
    return data instanceof ArrayBuffer
      ? data
      : bufferToArrayBuffer(data as Buffer);
  }
  ...
  async listVersions(fullPath: string): Promise<Entity[]> {
    ...
    if (!this.isNextcloud)
      throw new Error(
        "Versioning only supported for Nextcloud WebDAV currently"
      );
    ...
    const parser = new XMLParser({ removeNSPrefix: true });
    const result = parser.parse(res.text);
    const entries = Array.isArray(result?.multistatus?.response)
      ? result.multistatus.response
      : [result?.multistatus?.response].filter(Boolean);

    return entries
      .filter((e: any) => e.href.endsWith(fileId) === false)
      .map((e: any) => {
        const href = e.href as string;
        const verId = href.split("/").pop()!;
        const mtime = e.propstat?.prop?.getlastmodified
          ? Date.parse(e.propstat.prop.getlastmodified).valueOf()
          : Date.now();
        return {
          key: fullPath,
          keyRaw: fullPath,
          size: parseInt(e.propstat?.prop?.getcontentlength || "0", 10),
          sizeRaw: parseInt(e.propstat?.prop?.getcontentlength || "0", 10),
          mtimeSvr: mtime,
          mtimeCli: mtime,
          versionId: verId,
          isLatest: false,
...
export class WebdavFileSystem extends BaseCloudFs {
  constructor(
    ...
    super(
      kind,
      new RawWebdavFs(config, saveUpdatedConfigFunc),

...
export const getShrinkedSettings = (config: OnedriveConfig | OnedriveFullConfig) => {
...
class RawOneDriveFs implements RawFs {
  ...
  async stat(fullPath: string): Promise<Entity> {
    await this.ensureInited();
    const item = await this.api!.getItem(`${this.rootDrivePath}${fullPath === "/" ? "" : fullPath.replace(/\/$/, "")}`);
    const isDir = !!item.folder;
    return {
  ...
  async writeFile(fullPath: string, content: ArrayBuffer): Promise<Entity> {
    await this.ensureInited();
    const item = await this.api!.upload(`${this.rootDrivePath}${fullPath}`, content);
    return {
      key: fullPath, keyRaw: fullPath,
      sizeRaw: item.size || 0,
      mtimeSvr: Date.parse(item.lastModifiedDateTime).valueOf(),
  ...
  async checkConnect(fullPath: string): Promise<boolean> {
    await this.ensureInited();
    await this.api!.listChildren(`${this.rootDrivePath}${fullPath === "/" ? "" : fullPath.replace(/\/$/, "")}`);
    return true;
  }
}

...
function xorBlocks(output: Uint8Array, input1: Uint8Array, input2: Uint8Array) {
  if (input1.length !== input2.length) {
    throw Error(
      `input1.length=${input1.length} is not equal to input2.length=${input2.length}`
    );
  }
...
async function transform(
  ...
  if (m === 0 || m > 16 * 8) {
    throw Error(
      `EME operates on 1 to ${16 * 8} block-cipher blocks, you passed ${m}`
    );
  }

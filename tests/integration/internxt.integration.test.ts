...
import { isInternxtConfigured, testConfig } from "./config";
...
if (isInternxtConfigured) {
  describe("Internxt Integration Tests", function () {
    ...
    const getFs = () => {
      if (!fsInstance) {
        fsInstance = new RawInternxtFs(
          ...
          async () => {},
          { clientName: "remotely-save-tests", clientVersion: "0.0.1" }
        );
      }
      return fsInstance;
    };
    ...
    const rootPath =
      (testConfig.internxt.remoteBaseDir || "rs-test") + "-" + Date.now();
    ...
    describe("Internxt Large File & Robustness", () => {
      ...
      it("should handle simultaneous mkdir calls for the same path", async () => {
        ...
        await Promise.all([fs.mkdir(path), fs.mkdir(path), fs.mkdir(path)]);
